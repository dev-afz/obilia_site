...
var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;// Anm © 2014-2015 Andrey Polischuk
...
!function () {
  ...
  if (!elementClass) {
    console.log("Anm: data-anm attribute is not defined");
    return;
  }
  ...
  var msie = /MSIE.\d+\./gi.test(navigator.userAgent) && +navigator.userAgent.replace(/.*MSIE.(\d+)\..*/gi, "$1") < 9;
  ...
  function calculatePosition(e) {
    ...
    pos.fi = Math.atan(pos.x === 0 ? Infinity : -pos.y / pos.x) + (pos.x < 0 ? Math.PI : -pos.y < 0 ? 2 * Math.PI : 0);
    pos.s = 45 * Math.sin(2 * pos.fi) / 100;
    pos.x /= 100;
    pos.y /= 100;
    pos.r = Math.sqrt(Math.pow(pos.x, 2) + Math.pow(pos.y, 2)) / Math.sqrt(Math.pow(window.innerWidth, 2) + Math.pow(window.innerHeight, 2)) * 2;
    return pos;
  }
    ...
    if (msie) {
      ...
      for (var m in transformMap) {
        if (transformMap.hasOwnProperty(m)) {
          el.style[m] = [transformMap[m], "(", transform.x + "," + transform.y, transformMap[m] === "translate" ? "" : ",0", ") scale(", transform.scale, ") rotate(", transform.rotate, ")"].join("");
        }
      }
...
/******/ 	var __webpack_exports__ = __webpack_require__("./resources/js/anm.js");
/******/ 	
/******/ })()
;