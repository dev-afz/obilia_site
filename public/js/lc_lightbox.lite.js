...
(function ($) {
  ...
  // lightbox structure
  var lb_code = '<div id="lcl_wrap" class="lcl_pre_show lcl_pre_first_el lcl_first_sizing lcl_is_resizing">' + '<div id="lcl_window">' + '<div id="lcl_corner_close" title="close"></div>' + '<div id="lcl_loader" class="lcl_loader_pre_first_el"><span id="lcll_1"></span><span id="lcll_2"></span></div>' + '<div id="lcl_nav_cmd">' + '<div class="lcl_icon lcl_prev" title="previous"></div>' + '<div class="lcl_icon lcl_play"></div>' + '<div class="lcl_icon lcl_next" title="next"></div>' + '<div class="lcl_icon lcl_counter"></div>' + '<div class="lcl_icon lcl_right_icon lcl_close" title="close"></div>' + '<div class="lcl_icon lcl_right_icon lcl_fullscreen" title="toggle fullscreen"></div>' + '<div class="lcl_icon lcl_right_icon lcl_txt_toggle" title="toggle text"></div>' + '<div class="lcl_icon lcl_right_icon lcl_download" title="download"></div>' + '<div class="lcl_icon lcl_right_icon lcl_thumbs_toggle" title="toggle thumbnails"></div>' + '<div class="lcl_icon lcl_right_icon lcl_socials" title="toggle socials"></div>' + '</div>' + '<div id="lcl_contents_wrap">' + '<div id="lcl_subj">' + '<div id="lcl_elem_wrap"></div>' + '</div>' + '<div id="lcl_txt"></div>' + '</div>' + '</div>' + '<div id="lcl_thumbs_nav" class="lcl_pre_tn_scroll"></div>' + '<div id="lcl_overlay"></div>' + '</div>';

  ////////////////////////////////////////////////////////////////////

  // initialization
  // obj can be an array and overrides elements / [src: url/selector (only required data), title: (string), txt: (string), author: (string), ajax: bool, type: image/frame/text] 
  lc_lightbox = function lc_lightbox(obj, lcl_settings) {
    if (typeof obj != 'string' && (_typeof(obj) != 'object' || !obj.length)) {
      return false;
    }
  ...
  // destruct method
  lcl_destroy = function lcl_destroy(instance) {
  ...
  /* initialize */
  var lcl = function lcl(obj, settings) {
    var lcl_settings = $.extend({
      ...
      max_width: '93%',
      // Lightbox maximum width. Use a responsive percent value or an integer for static pixel value
      max_height: '93%',
      // Lightbox maximum height. Use a responsive percent value or an integer for static pixel value
      wrap_padding: false,
      // set lightbox wrapping padding. Useful to maintain spaces using px max-sizes. Use a CSS value (string)
      ol_opacity: 0.7,
      ...
      // bool/string / whether to use direct FB contents share (Read the doc to know what to use) 
      ...
      elems_parsed: function elems_parsed() {},
      html_is_ready: function html_is_ready() {},
      on_open: function on_open() {},
      on_elem_switch: function on_elem_switch() {},
      slideshow_start: function slideshow_start() {},
      slideshow_end: function slideshow_end() {},
      on_fs_enter: function on_fs_enter() {},
      on_fs_exit: function on_fs_exit() {},
    ...
    var lcl_vars = {
      elems: [],
      // elements object / src: url/text (only required data), title: (string), descr: (string), author: (string), type: image/iframe/text 
      is_arr_instance: typeof obj != 'string' && typeof obj[0].childNodes == 'undefined' ? true : false,
      // true if lightbox is initialized usign direct array immission
      elems_count: typeof obj != 'string' && typeof obj[0].childNodes == 'undefined' ? obj.length : $(obj).length,
      // elements count at the moment of lb initialization
      elems_selector: typeof obj == 'string' ? obj : false,
      // elements selector - used for dynamic elements fetching
      elem_index: false,
      // current element index
      gallery_hook_val: false,
      // gallery hook value - to discard other ones
      preload_all_used: false,
      // flag to know when complete preload on document's ready has been triggered
      img_sizes_cache: [],
      // store image sizes after their preload - index is images index

      inner_cmd_w: false,
      // store inner commands width for inner->outer switch 
      txt_exists: false,
      // any text exists in current element?
      txt_und_sizes: false,
      // custom lb sizes after text under calculation 
      force_fullscreen: false,
      // flag to know whether to simulate "always fs" for small screens
      html_style: '',
      // html tag style (for scrollbar hiding)
    ...
    var get_hash = function get_hash(str) {
    ...
    var obj_already_man = function obj_already_man(hash) {
      var found = false;
    ...
    var revert_html_entit = function revert_html_entit(str) {
      if (!str) {
        return str;
      }
    ...
    var attr_or_selector_data = function attr_or_selector_data($elem, subj_key) {
      var o = lcl_ai_opts;
      var subj = o[subj_key];
      if (subj.indexOf('> ') !== -1) {
    ...
    var setup_elems_obj = function setup_elems_obj($subj) {
      var o = lcl_ai_opts;
    ...
    var el_type_finder = function el_type_finder(src, forced_type) {
    ...
    var close_img_preload = function close_img_preload() {
    ...
    var maybe_preload = function maybe_preload(show_when_ready, el_index, cache_check) {
      var v = lcl_ai_vars;
    ...
    /* elements parsing */
    var elems_parsing = function elems_parsing(inst_obj, $clicked_obj) {
    ...
    /* open lightbox */
    var open_lb = function open_lb(inst_obj, $clicked_obj) {
    ...
    var rm_pre_show_classes = function rm_pre_show_classes() {
    ...
    /* setup lightbox code */
    var setup_code = function setup_code() {
      var o = lcl_ai_opts;
      var v = lcl_ai_vars;
      var wrap_classes = [];
      var css = '';
      ...
      if (o.cmd_position == 'inner' && o.ins_close_pos == 'corner') {
        css += '#lcl_corner_close {' + 'top: ' + (o.border_w + Math.ceil($('#lcl_corner_close').outerWidth() / 2)) * -1 + 'px;' + 'right: ' + (o.border_w + Math.ceil($('#lcl_corner_close').outerHeight() / 2)) * -1 + ';' + '}';
      ...
      $('head').append('<style type="text/css" id="lcl_inline_style">' + css + '#lcl_overlay {' + 'background-color: ' + o.ol_color + ';' + 'opacity: ' + o.ol_opacity + ';' + '}' + '#lcl_window, #lcl_txt, #lcl_subj {' + '-webkit-transition-duration: ' + o.animation_time + 'ms; transition-duration: ' + o.animation_time + 'ms;' + '}' + '#lcl_overlay {' + '-webkit-transition-duration: ' + o.open_close_time + 'ms; transition-duration: ' + o.open_close_time + 'ms;' + '}' + '.lcl_first_sizing #lcl_window, .lcl_is_closing #lcl_window {' + '-webkit-transition-duration: ' + (o.open_close_time - o.ol_time_diff) + 'ms; transition-duration: ' + (o.open_close_time - o.ol_time_diff) + 'ms;' + '}' + '.lcl_first_sizing #lcl_window {' + '-webkit-transition-delay: ' + o.ol_time_diff + 'ms; transition-delay: ' + o.ol_time_diff + 'ms;' + '}' + '#lcl_loader, #lcl_contents_wrap, #lcl_corner_close {' + '-webkit-transition-duration: ' + o.fading_time + 'ms; transition-duration: ' + o.fading_time + 'ms;' + '}' + '.lcl_toggling_txt #lcl_subj {' + /* delay to allow sizing on text hiding */
      '-webkit-transition-delay: ' + (o.fading_time + 200) + 'ms !important;  transition-delay: ' + (o.fading_time + 200) + 'ms !important;' + '}' + '.lcl_fullscreen_mode.lcl_txt_over:not(.lcl_tn_hidden) #lcl_txt, .lcl_fullscreen_mode.lcl_force_txt_over:not(.lcl_tn_hidden) #lcl_txt {' + /* fs txt margin when thumbs are shown */
      'max-height: calc(100% - 42px - ' + o.thumbs_h + 'px);' + '}' + '.lcl_fullscreen_mode.lcl_playing_video.lcl_txt_over:not(.lcl_tn_hidden) #lcl_txt,' + '.lcl_fullscreen_mode.lcl_playing_video.lcl_force_txt_over:not(.lcl_tn_hidden) #lcl_txt {' + /* fullscreen txt over margin when thumbs are shown */
      'max-height: calc(100% - 42px - 45px - ' + o.thumbs_h + 'px);' + '}</style>');
    ...
    var no_body_touch_scroll = function no_body_touch_scroll(selector) {
    ...
    var show_element = function show_element() {
      if (!lcl_shown) {
        return false;
      }
      var v = lcl_ai_vars;
    ...
    /* element has text ? */
    var elem_has_txt = function elem_has_txt(el) {
      return el.title || el.txt || el.author ? true : false;
    };

    /* populate lightbox */
    var populate_lb = function populate_lb(el) {
     ...
     * dimension = w or h
     */
    var css_size_to_px = function css_size_to_px(size, dimension, ignore_max) {
      var px = 0;
      var $wrap = $('#lcl_wrap');
    ...
    /* set element sizes */
    var size_elem = function size_elem(el, flags, txt_und_sizes) {
      ...
      if (_typeof(v.txt_und_sizes) == 'object') {
        w = v.txt_und_sizes.w;
        h = v.txt_und_sizes.h;
        if (el.type == 'image') {
    ...
    var txt_under_h = function txt_under_h(curr_w, curr_h, max_height, recursive_count) {
    ...
    var side_to_over_txt = function side_to_over_txt(el, treshold, w, h, side_txt_w) {
    ...
    var inner_to_outer_cmd = function inner_to_outer_cmd(el, window_width) {
      var o = lcl_ai_opts;
      var fs_mode = $('.lcl_fullscreen_mode').length ? true : false;
    ...
    var switch_elem = function switch_elem(new_el, slideshow_switch) {
      var v = lcl_ai_vars;
    ...
    var temp_slideshow_stop = function temp_slideshow_stop() {
      if (typeof lcl_slideshow == 'undefined') {
        return false;
      }
    ...
    var progbar_animate = function progbar_animate(first_run) {
      var o = lcl_ai_opts;
      if (!o.progressbar) {
        return false;
      }
    ...
    /* close lightbox */
    var close_lb = function close_lb() {
      if (!lcl_shown) {
        return false;
      }
    ...
    var enter_fullscreen = function enter_fullscreen(set_browser_status, on_opening) {
    ...
    var fs_img_manag = function fs_img_manag(img_sizes) {
    ...
    /* exit fullscreen */
    var exit_fullscreen = function exit_fullscreen(set_browser_status) {
    ...
    var exit_browser_fs = function exit_browser_fs() {
    ...
    var setup_thumbs_nav = function setup_thumbs_nav() {
    ...
    /* thumbs image preload */
    var thumbs_nav_img_preload = function thumbs_nav_img_preload(img_url, el_index, uniq_id) {
      $('<img/>').bind("load", function () {
        if (!lcl_ai_vars) {
          return false;
        }
    ...
    var thumbs_nav_elems_w = function thumbs_nav_elems_w() {
      var thumbs_w = 0;
    ...
    var thumbs_nav_arrows_vis = function thumbs_nav_arrows_vis() {
      if (!$('#lcl_thumbs_nav').length) {
        return false;
      }
    ...
    var thumbs_nav_arrows_opacity = function thumbs_nav_arrows_opacity() {
    ...
    var thumbs_nav_scroll_to_item = function thumbs_nav_scroll_to_item(elem_id) {
    ...
    var lcl_iframe_click = function lcl_iframe_click() {
    ...
    $(document).on('click', '.lcl_socials', function (e) {
      ...
      if (!$('.lcl_socials > div').length) {
        ...
        if (img) {
          code += '<a class="lcl_icon lcl_pint" onClick="window.open(\'http://pinterest.com/pin/create/button/?url=' + page_url + '&media=' + encodeURIComponent(img) + '&description=' + title + '\',\'sharer\',\'toolbar=0,status=0,width=575,height=330\');" href="javascript: void(0)"></a>';
        }
        code += '</div>';
    ...
    var touch_events = function touch_events() {
    ...
    var set_curr_vars = function set_curr_vars() {
      if (!lcl_curr_obj) {
        return false;
      }
    ...
    // open lightbox
    lcl_open = function lcl_open(obj, index) {
    ...
    // resize lightbox
    lcl_resize = function lcl_resize() {
    ...
    lcl_switch = function lcl_switch(new_el) {
      if (!lcl_shown || lcl_is_active || !set_curr_vars()) {
        return false;
      }
    ...
    // start slideshow
    lcl_start_slideshow = function lcl_start_slideshow(restart) {
    ...
    // stop slideshow
    lcl_stop_slideshow = function lcl_stop_slideshow() {
